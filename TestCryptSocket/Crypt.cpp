...
static void
des_key(const uint8_t  *key, uint32_t SK[32]) {
	size_t keysz = sizeof(*key);
	//const void * key = luaL_checklstring(L, 1, &keysz);
	if (keysz != 8) {
		throw( "Invalid key size %d, need 8 bytes", (int)keysz);
	}
	des_main_ks(SK, key);
}

char *
Crypt::desencode(const uint8_t * text,const uint8_t * key) {
	uint32_t SK[32];
	des_key(key, SK);

	size_t textsz = sizeof(*text);
...
char *
Crypt::desdecode(const uint8_t * text, const uint8_t * key) {
	uint32_t ESK[32];
	des_key(key, ESK);
	uint32_t SK[32];
	int i;
	...
	if ((textsz & 7) || textsz == 0) {
		...
		throw("Invalid des crypt text length %d", (int)textsz);
	}
	uint8_t tmp[SMALL_CHUNK];
	uint8_t *buffer = tmp;
...
void
Crypt::hexencode(const uint8_t * text, char * out) {
	static char hex[] = "0123456789abcdef";
	size_t sz = 8; //sizeof(*text);
	...
	int i;
	for (i = 0; i<sz; i++) {
		out[i * 2] = hex[text[i] >> 4];
		out[i * 2 + 1] = hex[text[i] & 0xf];
	}
...
static void
read64(const uint8_t *pubkey,const uint8_t * prikey, uint32_t xx[2], uint32_t yy[2]) {
	size_t sz = sizeof(*pubkey);
	//const uint8_t *x = (const uint8_t *)luaL_checklstring(L, 1, &sz);
	if (sz != 8) {
		//luaL_error(L, "Invalid uint64 x");
		//throw("Invalid uint64 x");
	}
	//const uint8_t *y = (const uint8_t *)luaL_checklstring(L, 2, &sz);
	sz = sizeof(*prikey);
	if (sz != 8) {
		//luaL_error(L, "Invalid uint64 y");
		//throw("Invalid uint64 y");
	}
	xx[0] = pubkey[0] | pubkey[1] << 8 | pubkey[2] << 16 | pubkey[3] << 24;
	xx[1] = pubkey[4] | pubkey[5] << 8 | pubkey[6] << 16 | pubkey[7] << 24;
	yy[0] = prikey[0] | prikey[1] << 8 | prikey[2] << 16 | prikey[3] << 24;
	yy[1] = prikey[4] | prikey[5] << 8 | prikey[6] << 16 | prikey[7] << 24;
}
...
char *
Crypt::hmac64(uint32_t x[2],uint32_t y[2]) {
	//uint32_t x[2], y[2];
	//read64(L, x, y);
	uint32_t result[2];
	hmac(x, y, result);
...
void
Crypt::dhsecret(char *pubkey,char *prikey,char *secret) {
	uint32_t x[2], y[2];
	read64((const uint8_t *)pubkey, (const uint8_t *)prikey, x, y);
...
void
Crypt::dhexchange(char *x) {
	size_t sz = sizeof(x);
	//const uint8_t *x = (const uint8_t *)luaL_checklstring(L, 1, &sz);
	//if (sz != 8) {
		//luaL_error(L, "Invalid dh uint64 key");
		//throw("Invalid dh uint64 key");
	//}
	uint32_t xx[2];
